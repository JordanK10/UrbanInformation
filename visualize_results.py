...
def visualize_dynamic_p_results(cbsa_name, cbsa_results, cbsa_df, output_dir="output_dynamic_p",plot=False):
    """
    ...
    l = 2 # Assuming l=2 as per the model's standard configuration
    optimal_gamma = np.log(l) + p_t_series * np.log(p_t_series) + (1 - p_t_series) * np.log(1 - p_t_series)

    p_t_map = pd.Series(p_t_series, index=p_t_years)
    losses = [res["loss"] for res in cbsa_results if res["loss"] is not None]
    ...
    for res in cbsa_results:
        ax.plot(res["year"][1:], res["x_mean_trajectory"], color='crimson', alpha=0.05)
    ...
    for res in cbsa_results:
        ...
        if not np.isnan(aligned_p_t).any():
            gamma_traj = transform_x_to_gamma_dynamic(res["x_mean_trajectory"], aligned_p_t)
            if plot:
    ...
    if plot:
        ...
        # --- Panel 4: Model Confidence (Loss Distribution) ---
        ax = axes[1, 1]
        if losses:
            ax.hist(losses, bins=50, color='black', density=True)
            ax.axvline(np.mean(losses), color='red', linestyle='--', label=f'Mean: {np.mean(losses):.2f}')
        ax.set_title("Model Loss Distribution", fontsize=14)
        ax.set_xlabel("Final Average Loss", fontsize=12)
...
def main():
    ...
    """
    results_dir = "analysis_output_dynamic_p"
    output_dir = "output_dynamic_p"
