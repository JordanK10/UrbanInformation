...
def main(method='vi', max_block_groups_per_cbsa=None):
    """
    Main function to run the dynamic-p analysis in parallel.
    """
    ...
    for cbsa_name, cbsa_df in cbsa_acs_data.items():
        ...
        tasks = []
        # Use groupby to process each block group individually
        for bg_fips, group in cbsa_df.groupby('block_group_fips'):
            # Ensure data is sorted by year for correct diff calculation
            group = group.sort_values('year')
